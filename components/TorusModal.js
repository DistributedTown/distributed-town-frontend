...
const TorusModal = (props) => {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        setState({ torusdirectsdk: torusdirectsdk });
      } catch (error) {
  ...
  const { selectedVerifier, loginHint, consoleText } = state;
  let emailField = "";
    ...
    emailField = (
      <div style={{ marginTop: "20px" }}>
        <input
          type="email"
          value={loginHint}
          onChange={(e) => setState({ loginHint: e.target.value })}
      ...
      <form onSubmit={login}>
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              setState({ selectedVerifier: e.target.value })
            }
          >
